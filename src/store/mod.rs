...
struct AppStore {
    data: HashMap<UserId, DialogEntity>,
}
...
impl AppStore {
    ...
    pub fn update(&mut self, id: &UserId, patch: DialogPatch) -> Option<&DialogEntity> {
    ...
    pub fn save(&mut self, user_dialog: DialogEntity, id: UserId) -> Option<UserId> {
        match self.data.insert(id, user_dialog) {
            Some(value) => Some(value.user_id),
            None => None,
        }
    }

    pub fn get(&self, id: &UserId) -> Option<&DialogEntity> {
        self.data.get(id)
    }

    pub fn delete(&mut self, id: &UserId) -> Option<()> {
        if let Some(_) = self.data.remove(id) {
            Some(())
        } else {
            None
        }
...
mod tests {
    ...
    fn dialog_is_saved_at_store() {
        ...
        assert!(store.save(dialog.clone(), dialog.user_id.clone()).is_none());
    ...
    fn missing_user() {
        ...
        let user_id: UserId = Faker.fake::<String>();
    ...
    fn delete_works() {
        let mut store = AppStore::new();
        let dialog = generate_start_dialog("user_id".to_string());
        store.save(dialog.clone(), "user_id".to_string());
